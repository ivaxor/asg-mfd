...
void buzzer_event_queue_handler_t::task(void *pvParameter)
{
    ...
    new_beep:
        ...
        case RESPAWN_NO_TOKENS:
            ESP_LOGD(TAG, "Playing RESPAWN_NO_TOKENS");
        ...
        case RESPAWN_TOKEN_DECREMENT:
            ESP_LOGD(TAG, "Playing RESPAWN_TOKEN_DECREMENT");
            gpio_set_level(BUZZER_PIN, 1);
        ...
        case RESPAWN_BATCH:
            ESP_LOGD(TAG, "Playing RESPAWN_BATCH");
            gpio_set_level(BUZZER_PIN, 1);
            if (xQueueReceive(queue, &beep_type, pdMS_TO_TICKS(300)) == pdTRUE)
                goto new_beep;
            gpio_set_level(BUZZER_PIN, 0);
        ...
        case SETUP_MODE:
            ESP_LOGD(TAG, "Playing SETUP_MODE");
        ...
        case LOW_BATTERY:
            ESP_LOGD(TAG, "Playing LOW_BATTERY");
            gpio_set_level(BUZZER_PIN, 1);
            if (xQueueReceive(queue, &beep_type, pdMS_TO_TICKS(300)) == pdTRUE)
                goto new_beep;
            gpio_set_level(BUZZER_PIN, 0);
            if (xQueueReceive(queue, &beep_type, pdMS_TO_TICKS(150)) == pdTRUE)
                goto new_beep;
            ...
            gpio_set_level(BUZZER_PIN, 1);
            if (xQueueReceive(queue, &beep_type, pdMS_TO_TICKS(50)) == pdTRUE)
                goto new_beep;
            gpio_set_level(BUZZER_PIN, 0);
            break;
